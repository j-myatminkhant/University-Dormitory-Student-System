...
public class MenuComponentController implements Initializable {
    ...
    private void student_btn_clicked(MouseEvent event) throws IOException {
        ...
        Stage manageStudent = new Stage();
        Parent root = FXMLLoader.load(getClass().getResource("/FXML/Student/Manage_Student.fxml"));
        Scene scene = new Scene(root);
        manageStudent.initStyle(StageStyle.TRANSPARENT);
        manageStudent.setScene(scene);
        manageStudent.show();
        manageStudent.setResizable(false);
        ...
        root.setOnMouseDragged(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event){
                manageStudent.setX(event.getScreenX() - xoffset);
                manageStudent.setY(event.getScreenY() - yoffset);
            }
        });
    }
